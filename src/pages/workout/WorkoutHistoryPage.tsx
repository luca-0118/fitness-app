import WorkoutHistoryItem from "../../components/listItems/WorkoutHistoryItem.tsx";
            ...
            <div className={"flex w-full h-full flex-col p-4 overflow-y-scroll no-scrollbar"}>
                <div className="pt-2">
                    {history.data.map((completedWorkout,idx) => (
                        <WorkoutHistoryItem key={idx} completedWorkout={completedWorkout} onClick={() => handleNavigate(completedWorkout.sessionUuid)} />
                    ))}
                </div>
            </div>
        </>
