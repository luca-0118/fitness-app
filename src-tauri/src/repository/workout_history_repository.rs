...
use crate::domain::{CompletedExercise, CompletedSet, CompletedWorkout, CompletedWorkouts, DetailedWorkout, Session, SessionExercise, Set};
...
impl WorkoutHistoryRepo for WorkoutHistoryRepository {
    ...
    fn get_single(&self, id: String) -> Result<DetailedWorkout, Error> {
        self.db.use_conn(|tx| {
            ...
            let mut workout = stmt.query_one([&id], |row| {
                Ok(DetailedWorkout {
            ...
            let exercises_iter = stmt.query_map([&id], |row| {
...
fn load_cardio_set(tx: &Transaction,set_id:&String) -> Result<Vec<CompletedSet>,Error> {
    let mut stmt = tx.prepare("SELECT time,distance,timeCompleted FROM completedCardioExercises WHERE completedExerciseId=?1")?;

    let set_iter = stmt.query_map(&[&set_id], |row| {
        Ok(CompletedSet::Timed {
            time: row.get(0)?,
            distance: row.get(1)?,
            time_completed: row.get::<_, String>(2)?,
        })
    })?;
...
fn load_weighted_set(tx: &Transaction,set_id:&String) -> Result<Vec<CompletedSet>,Error> {
    let mut stmt = tx.prepare("SELECT reps,weight,timeCompleted FROM completedWeightExercises WHERE completedExerciseId=?1")?;
