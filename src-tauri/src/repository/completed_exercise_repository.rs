...
use crate::{
    domain::{AddExerciseParams, AddTimedSetParams, AddWeighedSetParams, CompletedExerciseRepo},
...
impl CompletedExerciseRepo for CompletedExerciseRepository {
    ...
    fn add_exercise(&self, req: AddExerciseParams) -> Result<String, Error> {
        ...
        self.db.use_conn(|tx| {
            tx.execute(
                "INSERT INTO completedExercises(ID,sessionId,ExerciseID) VALUES(?1,?2,?3)",
                [
                    &history_id,
                    &req.session_id.clone(),
                    &req.exercise_id.clone(),
                ],
            )
        })?;
